-- Version strategy experiments so old and new bot logic are not mixed in analytics.

alter table trades
  add column if not exists strategy_version text default 'v1_mr_hybrid';

update trades
set strategy_version = 'v1_mr_hybrid'
where strategy_version is null;

drop function if exists get_top_pairs(text, integer, integer);
drop function if exists get_top_pairs(text, integer, integer, text);
drop function if exists get_worst_pairs(text, integer, integer);
drop function if exists get_worst_pairs(text, integer, integer, text);
drop function if exists get_strategy_stats(text, integer);
drop function if exists get_strategy_stats(text, integer, text);
drop function if exists get_regime_stats(text, integer);
drop function if exists get_regime_stats(text, integer, text);
drop function if exists get_macd_bias_stats(text, integer);
drop function if exists get_macd_bias_stats(text, integer, text);
drop function if exists get_rsi_zone_stats(text, integer);
drop function if exists get_rsi_zone_stats(text, integer, text);
drop function if exists get_hold_time_stats(text, integer);
drop function if exists get_hold_time_stats(text, integer, text);
drop function if exists get_exit_reason_stats(text, integer);
drop function if exists get_exit_reason_stats(text, integer, text);
...
as $$
  ...
  where t.user_id = p_user_id
    and t.status = 'CLOSED'
    and (p_strategy_version is null or coalesce(t.strategy_version, 'v1_mr_hybrid') = p_strategy_version)
